...
def handle_seat_click(seat_id):
    ...
    info = st.session_state.seats[seat_id]
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 2. 빈자리 클릭 ➔ 입실 처리
    if info["status"] == "빈자리":
        st.session_state.seats[seat_id] = {"status": "사용중", "user": current_user, "time": now_str}
        try:
    ...
    # 3. 사용중 좌석 클릭 ➔ 퇴실 처리
        ...
        st.session_state.seats[seat_id] = {"status": "빈자리", "user": "", "time": now_str}
        try:
